""" Módulos de renomeação de arquivos. """
...
def dividir_pdf(caminho_pdf: str, diretorio_destino: str, progresso, root):
    """Função para dividir um arquivo PDF em várias páginas."""
    try:
        with open(caminho_pdf, "rb") as arquivo_pdf:
            ...
            for pagina in range(num_paginas):
                ...
                novo_nome_pdf = f"{os.path.splitext(os.path.basename(caminho_pdf))[0]}_p{pagina + 1}.pdf"
...
def ler_nomes_arquivo(caminho_nomes: str) -> List[str]:
    """Função para ler nomes de arquivos."""
...
def renomear_arquivos(
    arquivos: List[str], lista_novos_nomes: List[str], progresso, root
) -> str:
    ...
    try:
        for index, (filepath, novo_nome) in enumerate(zip(arquivos, lista_novos_nomes)):
            diretorio_destino = os.path.dirname(filepath)
...
def iniciar_interface():
    ...
    root = tk.Tk()
    root.title("Renomear e Dividir Arquivos PDF")
    ...
    style.configure(
        "TLabel", font=("Helvetica", 12), background="lightblue", foreground="black"
    )
    style.configure("TProgressbar", thickness=20)

    arquivo_label = ttk.Label(root, text="Arquivos selecionados:")
    ...
    def dividir_pdf_callback():
        ...
        if caminho_pdf:
            ...
            if diretorio_destino:
                mensagem = dividir_pdf(caminho_pdf, diretorio_destino, progresso, root)
